...
import type { MigrationScenario } from "./migration-scenario"
...
describe("valid history", () => {
  it("passes and leaves nothing in the database", async () => {
    ...
    await verifyMigrations({
      ...
      scenarios: await loadMigrationScenarios<typeof validMigrations>(
        testingDir,
        "migrations/valid/*.scenario.ts",
      ),
    })
...
describe("scenarios", () => {
  ...
  const setup: Scenario["setup"] = async ({ db, schemaRef }) => {
  ...
  it("names the scenario and step when assertDown fails", async () => {
    const error = await getError(
      verify({
        name: "irreversible transformation",
        migration: "0002_valid_name_title_case",
        setup,
  ...
  it("rejects a scenario of a missing migration before verification", async () => {
    const error = await getError(
      verify({ name: "missing", migration: "0099_missing", setup } as unknown as Scenario),
    )
    expect(error.message).toBe(`Scenario "missing" references missing migration "0099_missing".`)
  })

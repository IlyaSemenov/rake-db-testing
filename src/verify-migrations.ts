...
export interface VerifyMigrationsOptions<TMigrations extends MigrationModules> {
  db: Db
  /** Migrator configuration of the project; `migrations` and `migrationsTable` are taken from it. */
  config: MigrateConfig & { migrations: TMigrations }
  scenarios?: readonly MigrationScenario<TMigrations>[]
 ...
 */
export async function verifyMigrations<TMigrations extends MigrationModules>({
  db,
  config,
  scenarios = [],
  searchPath = (schema) => schema,
}: VerifyMigrationsOptions<TMigrations>): Promise<void> {
  ...
  await withRollback(db, async () => {
    for (const [index, migration] of Object.keys(migrations).entries()) {
      ...
      await withRollback(db, async () => {
        ...
        for (const scenario of scenarios) {
          if (scenario.migration === migration) {
            await withRollback(db, () => runScenario(scenario, context, up, down))
          }
        }
...
async function runScenario(
  ...
  await withErrorContext(
    ...
    () => `Scenario "${scenario.name}" of migration "${scenario.migration}" failed at "${step}".`,
  )
}

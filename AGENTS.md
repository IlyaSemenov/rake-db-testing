# rake-db-testing Agent Guide

## Overview

Verify rake-db migration history and data migration scenarios on PostgreSQL.
...
- Keep shared test helpers in `testing/` and import them through `#testing`.
- Keep test migration sets in `testing/migrations/<set>/`, and give every migration key a set-specific name: rake-db caches loaded migrations by key for the whole test process.
- Name compile-only tests `*.type-test.ts`.
- Keep `src/index.ts` limited to explicit public exports.
- Keep the main entry free of `node:*` imports so it works in bundled code; put file-system helpers in `src/fs.ts`, published as `rake-db-testing/fs`.
...
Tests run against PostgreSQL from `DATABASE_URL`; copy `.env.sample` to `.env` and adjust it if needed.
Tests roll back all changes, so any database works.
